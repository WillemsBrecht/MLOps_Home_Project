...
def prepareMachines(ws): # prepare compute machine
    ...
    # create or connect to compute target
...
def prepareEnv(ws, env_name): # Create environment with pip and conda packages
    # create environment with docker for azure
...
def prepareTraining(script_folder, compute_target, environment): # prepare training src
    # get environment variables
    ENV_MODEL = json.loads(os.environ.get("ENV_MODEL"))
    ...
    # define arguments and training script for src
...
def main():
    ...
    # Prepare dataset, compute target, environment and src
